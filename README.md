...
- [x ] Mention two parts of Express that you learned about this week.
    We learned about middleware (things that execute before requests, in this case) and we learned about server-side routing, which is when you have multiple routes being handled by different files to handle requests for different data.

- [ x] What is Express Middleware?
    Express middleware is what makes our server, and then allows us to make requests, complete CRUD operations, and give that information back to the client-side.

- [ x] How can a client send data to an API?
    right now we can send data through the url (params and query strings), headers, and the body. For a client, the most common form will likely be through the body in the shape of a form.

- [ x] What can the API return to help clients know if a request was successful?
    a res.status in the 200's, and also some visual feedback if we want to!
